...
/// Function to add a GeoServer WFS vector layer as GeoJSON
function addVectorLayer(map, layername, dateTime) {
    // Construct the WFS GetFeature request URL
    const wfsUrl = `http://localhost:8080/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=${layername}&outputFormat=application/json`;

    // Fetch the vector data from GeoServer
    fetch(wfsUrl)
        .then(response => response.json()) // Parse the response as JSON (GeoJSON format)
        .then(data => {
            // Add the GeoJSON data to the map
            const vectorLayer = L.geoJSON(data, {
                // You can style the vector features
                ...
                // Optional: Attach popups or any other interaction on each feature
            ...
            // Optionally adjust map view to fit the layer bounds
            map.fitBounds(vectorLayer.getBounds());

            // Optional: Store this layer for further manipulation if needed
            newLayer = vectorLayer;  // Save the reference to the layer
        })
        .catch(error => {
            console.error('Error fetching vector data via WFS:', error);
        });
}
...
// Track layers added to each map
const viirsLayers = {};
const modisLayers = {};
const perimeterLayers = {};
...
if(layerType == 'VIIRS'){
    layername = 'Nelson:VIIRS_data'

    layerlist = viirsLayers;

}

if(layerType == 'MODIS'){
    layername = 'Nelson:Modis_data'
    layerlist =  modisLayers;

}

if(layerType == 'PERIMETER'){
    layername = 'Nelson:fire_perimeters'
    layerlist = perimeterLayers;
}


if (layerlist[mapId]) {
    // Remove the layer if it already exists
    map.removeLayer(layerlist[mapId]);
    delete layerlist[mapId];
    console.log(`overlay removed from ${mapId}`);
    } else {
    ...
    if(layername == 'PERIMETER'){
        addVectorLayer(mapId, layername, dateTime)
    }
    ...
    const newLayer = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        layers: layername, // Replace with the correct layer name from GeoServer
        format: 'image/png',
        transparent: true,
        version: '1.1.1',
        maxZoom: 18,
    ...
    newLayer.addTo(map);
    layerlist[mapId] = newLayer;
    console.log(` overlay added to ${mapId}`);
        }}
...
// Add event listeners for the Toggle VIIRS Overlay button
document.addEventListener('click', function(event) {
    if (event.target.classList.contains('fire-perimeters')) {
...
// Add event listeners for the Toggle VIIRS Overlay button
document.addEventListener('click', function(event) {
    if (event.target.classList.contains('toggle-fire-perimeters')) {
